...
if __name__ == '__main__':
    ...
    def drag_coeff(lon,lat,omega):
        ...
        if np.isnan(kh) == 0 and omega>f_loc and omega<N_loc:
            ...
            sigma_xx, sigma_yy, sigma_xy, h_rms, k_bar = np.nan, np.nan, np.nan, np.nan, np.nan
        ...
        return sigma_xx, sigma_yy, sigma_xy, h_rms, k_bar
    ...
    for j in range(ny):
        for i in range(nx):
            ...
            if np.isnan(get_N_clim(lon[j,i],lat[j,i])) == 0 and np.isnan(topog.z.interp(lat=np.arange(lat[j,i]-3,lat[j,i]+3),lon=np.arange(lon[j,i]-3,lon[j,i]+3)).mean(skipna=False)) == 0:
                sigma_xx[j,i], sigma_yy[j,i], sigma_xy[j,i], h_rms[j,i], k_bar[j,i] = drag_coeff(lon[j,i],lat[j,i],omega_M2)
    ...
    sigma_xx_xr = xr.DataArray(sigma_xx, dims=['yh','xh'],
                                         coords={'yh': yh,
                                                 'xh': xh},
                                         attrs={'long_name': 'x-dir drag coefficient, M2', 'units': 'second-1'})

    sigma_yy_xr = xr.DataArray(sigma_yy, dims=['yh','xh'],
                                         coords={'yh': yh,
                                                 'xh': xh},
                                         attrs={'long_name': 'y-dir drag coefficient, M2', 'units': 'second-1'})

    sigma_xy_xr = xr.DataArray(sigma_xy, dims=['yh','xh'],
                                         coords={'yh': yh,
                                                 'xh': xh},
                                         attrs={'long_name': 'xy-dir drag coefficient, M2', 'units': 'second-1'})
    ...
    sigma.to_netcdf('/g/data/nm03/lxy581/global_drag_coeff/sigma_SAH_M2_1deg_2d_%03d.nc' % (itile))
